...
{
#ifdef DEBUG
    scoped_timer t("argument_parser::parse()");
#endif
    ...
    try
    {
        ...
        desc.add_options()
            ...
            ("input,i", boost::program_options::value<std::vector<std::string>>(), "Name of input files to process into ascii (flag is optional, anything without a flag is considered an input file")
            ...
            ("invert,I", boost::program_options::bool_switch(&_parsed.invert)->default_value(false), "Inverts the ascii-calculation so that dark pixels are the lightest and vice versa")
